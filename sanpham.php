...
<div class="tong">
    ...
    <div class="content">
        <?php
        $sach = chitiet_sach($id_sach);
        $ct_sach = mysqli_fetch_array($sach);
        if(isset($_POST['ok']))
        {
            if(isset($_SESSION['id_user']))
            {
                $soluong = (int)$_POST['soluong'];
                if($soluong > $ct_sach['soluong'] || $soluong < 1)
                {
                    echo "<script>alert('Số lượng không hợp lệ')</script>";
                }
                else
                {
                    $kt = sach_giohang($_SESSION['id_user'],$ct_sach['id_sach']);
                    if(mysqli_num_rows($kt)!= 0)
                    {
                        $gh = mysqli_fetch_array($kt);
                        $tangsl = tang_slsach($soluong,$gh['id_giohang']);
                        if($tangsl) echo "<script>alert('Thêm vào giỏ hàng thành công')</script>";
                        else  echo "<script>alert('Lỗi')</script>";
                    }
                    else
                    {
                        $giohang = them_giohang($_SESSION['id_user'],$ct_sach['id_sach'],$soluong);
                        if($giohang) echo "<script>alert('Thêm vào giỏ hàng thành công')</script>";
                        else  echo "<script>alert('Lỗi')</script>";
                    }
                }
        ...
        <form method="post">
            <div class="sp">
                ...
                <div class="book">
                    <div class="anh">
                        <img src="img/<?php echo $ct_sach['hinhanh']; ?>" width="270px" height="364px">
                    </div>
                    <div class="info">
                        <h2>Tên sách: <?php echo $ct_sach['tensach']; ?></h2>
                        <span>Tác giả: <?php echo $ct_sach['tacgia']; ?></span>
                        <span>NXB: <?php echo $ct_sach['nxb']; ?></span>
                        <span>Giá: <?php echo $ct_sach['dongia']; ?> đồng</span>
                        <div class="muahang">

                            Số lượng <input type="text" name="soluong" value="1">
                            <input type="submit" name="ok" value="Chọn mua">

                        </div>
                        <div class="ghichu">
                            <?php echo $ct_sach['ghichu']; ?>
                        </div>
                    </div>
                    <div style="clear: both; padding-top: 40px; text-align: center; ">
                        <span style="padding: 10px; background: #9999CC; color: #FFFFFF;margin-left: 50px;">Tình trạng: <?php echo $ct_sach['soluong']; ?> sản phẩm</span>
                    </div>
                </div>
            </div>
        </form>
